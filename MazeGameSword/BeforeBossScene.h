...
// 剧情步骤
struct BossStoryStep {
    BossStoryStepType type;
    std::string background;  // 背景图片路径
    std::string leftImage;   // 左侧立绘路径
    std::string rightImage;  // 右侧立绘路径
    BossDialogue dialogue;   // 对话内容
    std::string text;        // 纯文本内容
};

class BeforeBossScene : public Scene {
private:
    SceneManager* sceneManager;

    // 剧情数据
    std::vector<BossStoryStep> steps;
    size_t currentStep;
...
public:
    explicit BeforeBossScene(SceneManager* manager);
    ~BeforeBossScene() override;
